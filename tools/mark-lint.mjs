...
// parcel's authority is geometric, so it needs every mark's rect to answer.
{
  ...
  // slug. The tool that projected it, tools/households-project.mjs, was DELETED by
  // G2 (P-145); its last output is the frozen fixture
  // WORLD/fixtures/households-declared-2026-08-10.json, and that file is what
  // --households wants until the store answers for the grain after G1. The flag is
  // how a caller points this gate at the correct grain, exactly as the fold's own
  // --households already does.
