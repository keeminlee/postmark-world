 ...
 * green.
 *
 * BEFORE THE SWEEP IS NOT BEFORE THE CROSSING, under a store crossing. On the
 * office train's store path this file is captured AFTER `store-writedown.mjs`
 * has already deleted every draft ref and rebuilt one sketchbook per household
 * (office `deploy/settlement-auto.sh`, write-down then `before.json`, then the
 * sweep). So `branches` here is never empty for a crossing that carries
 * candidates, and a rewind puts back the scratch surfaces the crossing made —
 * not the store rows it read them from. Phase 0's reach is bounded by that:
 * what it can say is what the TREE did, which is why `unheldChanges` is the
 * question it asks. Do not read a sketchbook count here as evidence of which
 * era ran; the era is known only in the office receipt's `source:` field.
 * (Measured against office `train/2026-w38` @ `a2bac38`.)
 */
