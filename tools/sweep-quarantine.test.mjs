// sweep-quarantine.test.mjs — one poisoned drawer must not refuse the whole town.
//
...
// the forecast fix uncovered the same class: quarantine the drawer, settle the
// town.
//
...
//                   BEFORE anything in that drawer is read, so nothing applies
 ...
 *  refuse, and the drawer becomes unfoldable. */
function town(t, { poisonRef = null } = {}) {
  ...
  // AN OVER-WITHDRAWAL on a mark that exists only in house-a's drawer. It passes
  // the sweep's markIds filter THERE (the mark is present on that ref) and is
  // filtered away everywhere else, so it poisons exactly one drawer — which is
...
test("the control: with no poisoned row, nothing is quarantined and both drawers settle", (t) => {
...
test("FALSIFIER: a poisoned ref is QUARANTINED while the other drawers settle", (t) => {
  ...
  assert.equal(out.quarantined.length, 1, "exactly the one bad drawer");
...
test("LOUD: the report names the ref, the reason, and the row that poisoned it", (t) => {
  ...
  assert.match(q.reason, /set aside/, "it says what happened to the drawer");
...
test("CONSERVATIVE: nothing from the quarantined drawer half-applies", (t) => {
  ...
  assert.equal(has("main", aPath), false, "the unfoldable drawer published nothing to main");
  for (const channel of ["published", "left_drafted", "unpublished", "withdrawn", "rehomed"])
    assert.ok(!JSON.stringify(out[channel] ?? []).includes("alice-shed"),
      `alice-shed leaked into ${channel} — a quarantined drawer must not be half-read`);
});
...
test("a quarantine is not a silence: the drawer is still THERE afterwards", (t) => {
  ...
  assert.match(branches, /draft\/house-a/,
    "setting a drawer aside must not delete it — the household fixes it and the next crossing takes it");
});
