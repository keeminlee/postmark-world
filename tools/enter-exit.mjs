...
//
// One line per ACT, append-only, in the walk ledger's own spine, so a reader who
// knows one knows both:
//
//   - <iso> · <handle> · enters <mark-id> · ferry <n> · word <m>
//   - <iso> · <handle> · exits  <mark-id> · ferry <n>
//
// ── THE NUMBER IS THE FERRY'S, AND NOW IT SAYS SO ───────────────────────────
//
// It used to be written `at <n>`, sitting one separator away from prose about
// walking through a door — two unrelated senses of one idea in a single line.
// The number has never had anything to do with this act: it is the FERRY's
// fractional crossing, the town clock's own unit, the thing the ferry counts
// twice a day. Naming it `ferry` costs one word and removes the collision.
//
// BOTH ERAS PARSE. Rows written before 2026-08-26 say `at` and are read exactly
// as they were written — a record is not editable, and a grammar change that
// orphaned 155 lines of history would be a rename storm paid for by the past.
// The writer emits `ferry` from here on; the reader accepts either, forever.
//
...
/** What the writer calls the ferry's number. The reader still takes `at`. */
export const FERRY_FIELD = "ferry";
...
export function formatEnterExit({ handle, act, mark, at, word = null, iso = null }) {
  ...
  return `- ${stamp} · ${handle} · ${act} ${mark} · ${FERRY_FIELD} ${stampAt(at).toFixed(4)}${said}`;
}
