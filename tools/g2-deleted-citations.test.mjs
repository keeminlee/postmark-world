...
// to run it. So the sweep is scoped to executable files and to the imperative — a
// `.md` may say a tool was deleted; a `.mjs` may not import it, and no comment
// anywhere may still instruct someone to invoke it.
//
// It can fail, and it failed before the commits that added it: at the world train tip
// `2714f92d` both tools existed, and four comments told a reader to run
// `households-project.mjs`.
